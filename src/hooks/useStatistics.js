...
export default function useStatistics({ dateFrom = '', dateTo = '', locationFilter = '' } = {}) {
  ...
  let salesQuery = fromPublic('sales').select('id, total_amount, sale_date, location_id, currency');
        ...
        // 3) Lay-By dues by currency using canonical view
        // NOTE: laybys table does not have location_id; derive via related sales already fetched.
        // We use the salesData (already filtered by date/location) and fetch laybys for those sale_ids.
        let laybyData = [];
        if ((salesData || []).length) {
          const saleIdsForLocation = (salesData || []).map(s => s.id);
          const { data: laybyRows, error: laybyError } = await supabase
            .from('laybys')
            .select('id, sale_id')
        ...
        if (saleIdsForLayby.length) {
          const { data: vfs } = await supabase
            .from('v_sales_financials')
            .select('sale_id,currency,outstanding_amount')
            .in('sale_id', saleIdsForLayby);
          (vfs || []).forEach(v => {
            const curRaw = v.currency || 'K';
            const cur = (curRaw === '$' || (curRaw || '').toUpperCase() === 'USD') ? 'USD' : 'K';
            const due = Math.max(0, Number(v.outstanding_amount || 0));
            laybyByCurrency[cur] = (laybyByCurrency[cur] || 0) + due;
            if (cur === 'K') dueK += due; else if (cur === 'USD') due$ += due;
          });
        }
