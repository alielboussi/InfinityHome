...
import { fetchCarpentryProducts } from '../services/products';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError('');
        const rows = await fetchCarpentryProducts();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Carpentry Products</Text>
      ...
      {loading ? (
        <ActivityIndicator style={styles.centered} size="large" color="#0f766e" />
      ) : error ? (
...
const styles = StyleSheet.create({
  ...
  cartButtonText: { color: '#0f766e', fontWeight: '700' },
  search: {
    margin: 12,
    backgroundColor: '#fff',
    borderRadius: 8,
  ...
  add: { fontSize: 24, color: '#0f766e', fontWeight: '700', paddingLeft: 12 },
